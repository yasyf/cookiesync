...
**Search before writing.** Before creating a helper, query the codebase via `ccx code search` (intent) or `ccx code symbol` (a named symbol). Sibling modules and base packages win over re-implementation — check `github.com/yasyf/synckit` before re-implementing generic sync plumbing here.
