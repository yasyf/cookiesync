...
// code and stdout so callers branch on the documented 0 (success) / 1
...
// the helper wrote to stdout. Stderr is captured separately and surfaced only when
// a call fails to run, since the helper logs diagnostics there.
type Result struct {
	Code   int
	Stdout []byte
}
...
// or the encrypt failed. plaintext and the returned blob are raw bytes.
...
// or the decrypt failed. blob and the returned plaintext are raw bytes.
...
// extraEnv to the inherited environment. It returns the helper's exit code and
// stdout; a non-zero exit is reported in Result.Code, not as an error, so callers
// branch on the 0/1/2 contract. err is non-nil only when the helper cannot be
// resolved or spawned, or it dies on a signal.
func (b Bridge) run(ctx context.Context, stdin []byte, extraEnv []string, args ...string) (Result, error) {
	...
	case runErr == nil:
		return Result{Code: 0, Stdout: stdout.Bytes()}, nil
	case isExit(runErr):
		var exitErr *exec.ExitError
		errors.As(runErr, &exitErr)
		return Result{Code: exitErr.ExitCode(), Stdout: stdout.Bytes()}, nil
	default:
