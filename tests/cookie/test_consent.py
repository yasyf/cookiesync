...
HELPER = Path("/fake/cookiesync-keyhelper.app/Contents/MacOS/cookiesync-keyhelper")


class FakeRunner:
    """A stand-in for ``anyio.run_process`` that scripts each command's result by verb.

    ``vault-status``/``vault-retrieve``/``vault-enroll`` are dispatched on ``argv[1]``;
    the ``security`` fallback on ``argv[0]``. Every invocation is recorded so tests can
    assert call counts.
    """
...
def patched_helper(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(paths, "require_helper", lambda: HELPER)
...
async def test_retrieve_returns_derived_key_and_prompts_once(
    ...
    assert runner.verb_count("vault-enroll") == 0
    assert runner.verb_count(consent.SECURITY) == 0
    status = next(argv for argv in runner.calls if argv[1] == "vault-status")
    assert status == [str(HELPER), "vault-status", VAULT]
...
async def test_invalidated_vault_reenrolls_then_retrieves(
    ...
    async def scripted_retrieve(
        ...
        if argv[1] == "vault-retrieve":
            code, out = next(retrieve_results)
            return subprocess.CompletedProcess(argv, code, stdout=out, stderr=b"")
        if argv[1] == "vault-enroll":
    ...
    assert key == derive_key(SafeStorageKey(PASSWORD))
    assert runner.verb_count("vault-enroll") == 1
    assert runner.verb_count("vault-retrieve") == 2
...
async def test_obtain_key_unprompted_does_a_bare_security_read_no_touch_id(
    ...
    # and never invokes the Touch ID vault helper (vault-status/vault-retrieve/vault-enroll).
    runner = FakeRunner(
        status=(0, b"biometry=true passcode=true vault=true\n"),
        retrieve=(0, b"should-not-be-used"),
        security=(0, PASSWORD.encode() + b"\n"),
    ...
    assert runner.verb_count(consent.SECURITY) == 1
    assert runner.verb_count("vault-retrieve") == 0
    assert runner.verb_count("vault-status") == 0
    assert runner.verb_count("vault-enroll") == 0
    security = next(argv for argv in runner.calls if argv[0] == consent.SECURITY)
    assert security == [consent.SECURITY, "find-generic-password", "-w", "-s", CHROME.keychain_service]


async def test_obtain_key_fails_closed_when_helper_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    # The signed helper is absent: obtain_key fails closed via require_helper rather than
    # degrading to an unsigned/ad-hoc build. The Touch-ID prompt never runs.
    def absent() -> Path:
        raise HelperError("cookiesync key helper not found; run 'cookiesync install'")

    monkeypatch.setattr(paths, "require_helper", absent)

    async def fail(*_: object, **__: object) -> object:
        raise AssertionError("no subprocess should run when the helper is missing")

    monkeypatch.setattr(consent.anyio, "run_process", fail)
